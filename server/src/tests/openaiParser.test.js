...
describe('OpenAI Parser', () => {
  ...
  test('should throw error for empty input', async () => {
    await expect(parseTaskWithAI('')).rejects.toThrow('Input must be a non-empty string');
  });
  ...
  test('should throw error when OpenAI API fails', async () => {
    ...
    await expect(parseTaskWithAI('Test task')).rejects.toThrow('Failed to parse task with AI');
  });
});